...
import com.xy.music.db.MusicInfoDao;
import com.xy.music.fragment.MainFragment;
import com.xy.music.fragment.MenuFragment;
import com.xy.music.slidemenu.SlidingMenu;
import com.xy.music.utils.CrashUtil;
import com.xy.music.utils.MusicUtils;
import com.xy.music.utils.SocketUtil;
import com.xy.music.utils.SplashScreen;
...
public class HomeActivity extends FragmentActivity implements IConstants {
	...
	protected void onCreate(Bundle arg0) {
		...
		SocketUtil.startSocketService(HomeActivity.this); // 启动socket长连接服务
		isSDCardWork(); // 初始化检查SD卡是否挂载
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (mMusicDao.hasData()) {
					...
					MusicUtils.queryMusic(HomeActivity.this,START_FROM_LOCAL);
					MusicUtils.queryAlbums(HomeActivity.this);
					MusicUtils.queryArtist(HomeActivity.this);
					MusicUtils.queryFolder(HomeActivity.this);
	...
	private final BroadcastReceiver sdCardReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("android.intent.action.MEDIA_REMOVED")// 各种未挂载状态
				...
				finish();
				Toast.makeText(HomeActivity.this, "SD卡已拔出或失效",
	...
	public void showSleepDialog() {
		if (AppManager.mIsSleepClockSetting) {
			cancleSleepClock();
		...
		final Dialog dialog = new Dialog(HomeActivity.this, R.style.round_corner_dialog);
	...
	private void setSleepClock(String timeS) {
		...
		PendingIntent pendingIntent = PendingIntent.getBroadcast(
				HomeActivity.this, 0, intent, 0); // 定时任务完成后会发送一个广播，广播接收者会结束掉应用
		// 设置time时间之后退出程序
		int time = Integer.parseInt(timeS);
		long longTime = time * 60 * 1000L;
		AlarmManager am = (AlarmManager) getSystemService(ALARM_SERVICE);
		am.set(AlarmManager.RTC, System.currentTimeMillis() + longTime, pendingIntent); // 设置定时任务
		AppManager.mIsSleepClockSetting = true;
		Toast.makeText(getApplicationContext(), "将在"+timeS+"分钟后退出", Toast.LENGTH_SHORT)
				.show();
	}
	...
	private void cancleSleepClock() {
		...
		AppManager.mIsSleepClockSetting = false;
	}
	...
	public void onDestroy() {
		...
		AppManager.mServiceManager.exit();
		AppManager.mServiceManager = null;
