...
import com.xy.music.activity.PlayQueueActivity;
import com.xy.music.adapter.LyricAdapter;
import com.xy.music.adapter.MyAdapter;
import com.xy.music.db.FavoriteInfoDao;
import com.xy.music.db.MusicInfoDao;
import com.xy.music.lrc.LyricDownloadManager;
import com.xy.music.lrc.LyricLoadHelper;
import com.xy.music.lrc.LyricLoadHelper.LyricListener;
import com.xy.music.model.LyricSentence;
...
import com.xy.music.view.MySlidingDrawer;
...
public class SlidingDrawerManager implements OnClickListener,
	...
	private void showLrcDialog() {
		...
		final Dialog dialog = new Dialog(mActivity, R.style.round_corner_dialog);
...
//						loadLyric(music, artist);
	...
	public void loadLyric(MusicInfo playingSong) {
		...
		// 取得歌曲同目录下的歌词文件绝对路径
		String lyricFilePath = AppManager.lrcPath + "/" + playingSong.musicName
				+ ".lrc";
	...
	private void loadLyricByHand(String musicName, String artist) {
		// 取得歌曲同目录下的歌词文件绝对路径
		String lyricFilePath = AppManager.lrcPath + "/" + musicName + ".lrc";
		File lyricfile = new File(lyricFilePath);
